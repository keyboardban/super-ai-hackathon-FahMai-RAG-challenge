...
# === CHECKPOINT FILES ===
EMBEDDINGS_CACHE = "embeddings_cache_basic_loop.npy"
BACKUP_SUBMISSION = "backup_submission_basic_loop.csv"
...
# === 1. DATA PREPARATION (STARTER KIT STYLE) ===
...
print(f"\n=== PHASE 2: BASIC RETRIEVAL + LOOP REASONING ===")
...
for q in questions[:N_QUESTIONS]:
    ...
    # 1. STARTER KIT RETRIEVAL (NO REWRITE, NO RERANKER, TOP 5 ONLY)
...
with open("basic_loop_submission.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["id", "answer"])
    writer.writerows(results)

print("BASIC LOOP pipeline complete! Saved to basic_loop_submission.csv")
