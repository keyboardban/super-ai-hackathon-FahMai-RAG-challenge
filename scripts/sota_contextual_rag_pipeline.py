...
USE_CONTEXTUAL_ENRICHMENT = True # THE SOTA FEATURE (Requires ~16 Hours)

# === CHECKPOINT FILES ===
CONTEXTUAL_CACHE = "contextual_chunks_checkpoint_sota.json"
EMBEDDINGS_CACHE = "embeddings_cache_sota.npy"
BACKUP_SUBMISSION = "backup_submission_sota.csv"
...
def generate_contextual_chunk(doc_text, chunk_text):
    system_prompt = "คุณคือ AI ผู้ช่วยสรุปข้อมูล จงเขียนบริบทสั้นๆ 1-2 ประโยคอธิบายว่าส่วนย่อยนี้พูดถึงอะไร เพื่อใช้เป็นบริบทค้นหา"
    user_prompt = f"[เอกสารฉบับเต็ม]\n{doc_text[:1500]}...\n\n[ส่วนย่อย (Chunk)]\n{chunk_text}\n\nจงสรุปบริบทสั้นๆ ปะหน้าข้อความส่วนย่อยนี้:"
...
def get_or_build_chunks():
    ...
    if kb_dir.exists():
        for fp in sorted(kb_dir.rglob("*.md")):
            ...
            documents.append({
                ...
                "text": unroll_markdown_tables(raw_text)
            })

    # Increase Chunk Size from 400 to 800
    ...
    if USE_CONTEXTUAL_ENRICHMENT:
        print("Starting Contextual Retrieval Enrichment... This will take time!")
...
def rewrite_query(question):
    # Temperature 0.1 for maximum stability
...
with open("sota_contextual_submission.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["id", "answer"])
    writer.writerows(results)

print("SOTA Contextual pipeline complete! Saved to sota_contextual_submission.csv")
