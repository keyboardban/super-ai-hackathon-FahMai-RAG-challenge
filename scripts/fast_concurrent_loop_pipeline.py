...
TOP_K = 5
MAX_WORKERS = 5 # <--- เพิ่มความเร็วด้วยการรัน 5 ข้อพร้อมกัน

# === CHECKPOINT FILES ===
EMBEDDINGS_CACHE = "embeddings_cache_fast_loop.npy"
BACKUP_SUBMISSION = "backup_submission_fast_loop.csv"
FINAL_SUBMISSION = "fast_loop_submission.csv"

# Global lock ป้องกันไฟล์พังเวลาเขียนลงไฟล์พร้อมๆ กัน
...
# === 3. RETRIEVAL (STARTER KIT HYBRID) ===
...
def process_question(q):
    ...
    MAX_LOOPS = 2 # ลดจาก 3 เป็น 2 เพื่อประหยัดเวลาให้มากขึ้น
    ...
    # บันทึกลงไฟล์อย่างปลอดภัย (ป้องกันไฟล์พังเวลา 5 กระบวนการเขียนพร้อมกัน)
    with csv_lock:
...
if __name__ == "__main__":
    ...
    print(f"\n=== PHASE 2: CONCURRENT BASIC RETRIEVAL + LOOP REASONING ===")
    
    questions_to_process = [q for q in questions[:N_QUESTIONS] if q["id"] not in done_qids]
    print(f"Running pipeline on {len(questions_to_process)} questions using {MAX_WORKERS} concurrent workers...")
    ...
    if questions_to_process:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            # Tqdm Progress bar แสดงภาพรวมเวลาแต่ละข้อสุ่มเสร็จ
    ...
    # เรียงตามรหัสข้อสอบเพื่อความถูกต้องตอนเขียนไฟล์รวม
    ...
    print(f"CONCURRENT BASIC LOOP pipeline complete! Saved to {FINAL_SUBMISSION}")
