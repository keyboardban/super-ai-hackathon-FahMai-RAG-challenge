...
BACKUP_SUBMISSION = "backup_submission_ultimate.csv"
...
for q in questions[:N_QUESTIONS]:
    ...
    # Give the LLM a massive context window of 10 chunks directly from Hybrid Search
    final_top10_idx = retrieve_top_k(rewritten_q, fetch_k=10)
...
# Write Final Output
with open("ultimate_submission.csv", "w", newline="", encoding="utf-8") as f:
    writer = csv.writer(f)
    writer.writerow(["id", "answer"])
    writer.writerows(results)

print("ULTIMATE pipeline complete! Saved to ultimate_submission.csv")
